...
export default async function MovieInfo({
  ...
  const translatedGenres = movie.genre?.map(
    (genre) =>
      t(`Genres.${genre.toLowerCase()}` as "Genres.historical") || genre,
  );
  const hours =
    movie.movieTime || 0 >= 60 ? Math.floor(movie.movieTime || 0 / 60) : 0;
  const minutes =
    hours * 60 !== movie.movieTime ? movie.movieTime || 0 - hours * 60 : 0;
  return (
